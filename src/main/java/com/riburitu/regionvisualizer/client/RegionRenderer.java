...
public class RegionRenderer {
    ...
    private static Region viewedRegion = null;
    
    private static boolean isHoldingRegionSelector(Player player) {
        ItemStack heldItem = player.getMainHandItem();
        return heldItem.getItem() instanceof RegionSelectorItem;
    }
    
    public static void setViewedRegion(Region region) {
        viewedRegion = region;
    }
    public static void clearViewedRegion() {
        viewedRegion = null;
    }
    ...
    public static void onRenderWorld(RenderLevelStageEvent event) {
        ...
        if (sel != null && sel.isComplete()) {
            ...
            if (selectionChanged) {
                lastSelection = new RegionSelection();
            ...
            if (wasInside == null || wasInside != inside) {
                if (currentTime - lastMessageTime > MESSAGE_COOLDOWN) {
                    ...
                    if (inside) {
                    	Minecraft.getInstance().gui.setOverlayMessage(Component.literal("Estás DENTRO de la región").withStyle(ChatFormatting.GREEN), false);;
                    }
                }
            }
